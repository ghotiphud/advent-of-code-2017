...
/// on one line, with the current instruction marked in parentheses.
...
fn part1(s: &str) -> i32 {
    let mut jumps: Vec<i32> = s.lines().map(|l| l.parse().unwrap()).collect();
    let len = jumps.len();
